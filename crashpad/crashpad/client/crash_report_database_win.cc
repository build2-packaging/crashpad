...
#else
      LOG(ERROR) << "(Internal error: mini_chromium build2-package does not support cout wide char.)" << " expected to start with "
                 << base::WideToUTF8(report_dir_.value());
#endif
      return;
    }
...
#else
        LOG(ERROR) << "unexpected attachment dir name "
                   << "(Internal error: mini_chromium build2-package does not support cout wide char.)";
#endif
        continue;
      }
